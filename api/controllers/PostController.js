            ...
            .upload({ maxBytes: 10000000 }, async (err, uploadedFiles) => {
              if (err) {
                ...
                try {
                  ...
                  return res
                    .status(404)
                    .json({ message: "post not stored", error: error + "hello" });
                }
              }
            });
        } catch (error) {
          return res.json({ message: "post not stored!!", error: error + "hello" });
        }
      },
//user like post 
...
if(owner=user.id){
        ...
        const like= await Post.update({id:id}).set({like:id+"like"})
}
